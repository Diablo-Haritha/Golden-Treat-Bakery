...
// --- Query: aggregate order_items and returns, expose total_amount and unit price ---
$sql = "
SELECT 
  o.id,
  o.order_date,
  ...
  -- gross_value: sum of (unit_price * qty) if items exist, else orders.total_amount
  COALESCE(ROUND(SUM(oi.unit_price * oi.quantity),2), o.total_amount, 0.00) AS gross_value,
  COALESCE(r.sum_qty,0) AS returned_qty,
  GREATEST(COALESCE(SUM(oi.quantity), o.quantity) - COALESCE(r.sum_qty,0), 0) AS net_quantity,
  -- net_value: for now same as gross_value (subtract refunds if you track them)
  COALESCE(ROUND(SUM(oi.unit_price * oi.quantity),2), o.total_amount, 0.00) AS net_value,
  -- total_amount: alias for backward compatibility with templates
  COALESCE(ROUND(SUM(oi.unit_price * oi.quantity),2), o.total_amount, 0.00) AS total_amount,
  -- price: computed unit price if items exist, else fallback to o.price
  COALESCE(
    ROUND(
      SUM(oi.unit_price * oi.quantity) / NULLIF(SUM(oi.quantity),0)
    ,2),
    o.price,
    0.00
  ) AS price,
...
foreach ($rows as $r) {
    ...
    $netQty = (int)($r['net_quantity'] ?? 0);
    $priceGross = (float)($r['gross_value'] ?? 0.0);
    $netValue = (float)($r['net_value'] ?? 0.0);
                  ...
                  <tr data-status="<?= htmlspecialchars(strtolower($r['status'])) ?>">
                    ...
                    <td class="row-actions">
                      ...
                      <button class="viewBtn" type="button"
                        ...
                        data-total-amount="<?= htmlspecialchars(number_format((float)$r['total_amount'],2,'.','')) ?>"
                      ...
                      <button class="invoiceBtn" type="button"
                        ...
                        data-product="<?= htmlspecialchars($r['product']) ?>"
                        data-quantity="<?= (int)$r['quantity'] ?>"
                        data-total_amount="<?= htmlspecialchars(number_format((float)$r['total_amount'],2,'.','')) ?>"
                        data-returned="<?= (int)$r['returned_qty'] ?>"
                        data-net-qty="<?= (int)$r['net_quantity'] ?>"
                        data-net-value="<?= htmlspecialchars(number_format((float)$r['net_value'],2,'.','')) ?>"
                        data-price="<?= htmlspecialchars(number_format((float)$r['price'],2,'.','')) ?>"
...
//JS to show item breakdown in the modal
  document.addEventListener('click', function(e){
  const btn = e.target.closest('.btnview, .viewBtn');
  if(!btn) return;
  const items = btn.dataset.items || '';
  const itemLines = items ? items.split(' || ') : [];
  const listHtml = itemLines.map(x => '<div>' + x + '</div>').join('');
  // update modal element where you want to show breakdown:
  const productEl = document.getElementById('v_product') || document.querySelector('#v_product');
  if (productEl) productEl.innerHTML = listHtml || '—';
  // fill other fields
  const idEl = document.getElementById('v_id'); if (idEl) idEl.textContent = btn.dataset.id || '';
  const qtyEl = document.getElementById('v_quantity'); if (qtyEl) qtyEl.textContent = btn.dataset.quantity || '';
  const priceEl = document.getElementById('v_price'); if (priceEl) priceEl.textContent = btn.dataset.total || '';
  // open modal (however your modal opens currently)
...
document.addEventListener('click', function(e){
  ...
  if (view) {
    ...
    openView({
      ...
      quantity: d.quantity,
      returned: d.returned,
      netQty: d.netQty || d.netQty, // dataset.netQty => dataset.netQty
      netValue: d.netValue,
      price: d.price,
