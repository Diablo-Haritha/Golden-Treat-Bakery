...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (isset($_POST['action'])) {
        if ($_POST['action'] == 'add') {
            ...
            if (empty($lastActive)) {
                $message = "Please select a last active date";
                ...
                if ($result->num_rows > 0) {
                    ...
                    $sql = "INSERT INTO users (full_name, email, mobile, address, district, role, date_joined, status, password)
                            VALUES ('$name', '$email', '$mobile', '$address', '$district', '$role', '$lastActive', '$status', '$password')";
            ...
            if (empty($lastActive)) {
                ...
                $sql = "UPDATE users SET 
                        full_name='$name', email='$email', mobile='$mobile', address='$address', 
                        district='$district', role='$role', date_joined='$lastActive', status='$status' 
                        WHERE id=$id";
                if ($conn->query($sql) === TRUE) {
            ...
            fputcsv($output, ['User ID', 'Name', 'Email', 'Mobile', 'Address', 'District', 'Role', 'Status', 'Last Active']);
            while ($row = $result->fetch_assoc()) {
                fputcsv($output, [
                    $row['id'], $row['full_name'], $row['email'], $row['mobile'], $row['address'],
                    $row['district'], $row['role'], $row['status'], $row['date_joined']
                ]);
            }
            fclose($output);
            exit();
...
if ($status) $conditions[] = "status = '$status'";
if ($search) {
    $conditions[] = "(id LIKE '%$search%' OR full_name LIKE '%$search%' OR email LIKE '%$search%' OR mobile LIKE '%$search%' OR address LIKE '%$search%' OR district LIKE '%$search%' OR role LIKE '%$search%')";
}
$where = $conditions ? "WHERE " . implode(" AND ", $conditions) : "";
    ...
    <div class="header-middle">
      ...
      <div class="search-bar">
        <form method="GET" action="">
          <input type="text" id="globalSearch" name="globalSearch" placeholder="Search by name, email, mobile, or role..." value="<?php echo htmlspecialchars($search); ?>">
        </form>
      </div>
    </div>
    <div class="header-right">
      <button class="role-btn" onclick="window.location.href='logout.php'">Logout</button>
  ...
  <div class="layout">
    <aside class="sidebar">
      ...
      <nav>
        ...
        <p>User Management</p>
        <div class="salebtn">
          <button class="tab-btn active" onclick="window.location.href='?panel=user-dashboard'">User Dashboard</button>
          <button class="tab-btn" onclick="window.location.href='?panel=user-export'">Export Report</button>
        </div>
      </nav>
    </aside>
