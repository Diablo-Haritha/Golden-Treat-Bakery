...
// ---------- ALL SALES DATA FOR REPORT ----------
$sql = "SELECT id, customer, total, status, date FROM sales ORDER BY date DESC";
$result = $conn->query($sql);
$salesData = [];
...
<body>
    ...
    <main class="free-area">
      <section id="sales-dashboard" class="panel active">
        <div class="content">
          ...
          <button class="generate-btn" id="generateReport">Generate Sales Report</button>

          <!-- Report View -->
          <div id="reportView">
            <h2>Sales Report - Summary & Details</h2>
            ...
            <!-- Sales Details Table -->
            <h3 style="color: #ffdd57; text-align: center;">All Sales Details</h3>
            <table id="salesTable">
              <thead>
                <tr>
                  <th>ID</th>
            ...
            <!-- Download PDF Button -->
            <button class="report-btn" id="downloadPDF">Download Report as PDF</button>
          </div>
        </div>
      </section>
    </main>
  ...
  <script>
    ...
    document.getElementById('downloadPDF').addEventListener('click', () => {
      ...
      let yPos = 40;
      doc.setFontSize(12);
      doc.text(`Generated on: <?= date('Y-m-d H:i:s') ?>`, 14, yPos);
      yPos += 10;

      const summaryKeys = Object.keys(summary);
      summaryKeys.forEach(key => {
        const label = key.replace(/([A-Z])/g, ' $1').replace(/^./, str => str.toUpperCase());
        doc.text(`${label}: ${summary[key]}`, 14, yPos);
        yPos += 7;
      });
      ...
      // Table Data
      const tableColumn = ['ID', 'Customer', 'Total (Rs.)', 'Status', 'Date'];
      const tableRows = salesData.map(row => [
        row.id,
        row.customer,
        `Rs. ${parseFloat(row.total).toFixed(2)}`,
        row.status,
